...
//Step 1, let's estimate the blur kernal. This allows the user to input what they think is the blur
void InverseFilter(Mat Y){
float blur[3][3];
for (int i = 0; i <= 2; i++){
     for (int j = 0; j < = 2; j++){
           cin >> blur[i][j];
...
//Step 2 we take the dft of the guesstimated blur kernal
dft(H, H);
//Step 3 we take the dft of the output signal
dft(Y, Y);
//Step 4 we divide output by the blur to get an estimation for the original signal
Mat estimatedX=Y/H;
//Step 5 perform a recerse DFT of the estimate.
dft(estimatedX,estimatedX, DFT_INVERSE);
//At this point, estiated X should be a matrix with an attempt at deblurring
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
